...
public class LibroMapper {
    public static List<LibroListDto> libroToLibroListDto(List<Libro> libros) {
        ...
        for (Libro libro: libros) {
            LibroListDto libroListDto = new LibroListDto(libro.getIdLibro(), libro.getTitulo(), libro.getUrlImagenChica());
    ...
    public static Optional<LibroDto> libroToLibroDto(Optional<Libro> libro) {
        if(libro.isPresent()) {
            ...
            generosDto = GeneroMapper.generoLibroToGeneroDto(libro.get().getGeneroLibros());
